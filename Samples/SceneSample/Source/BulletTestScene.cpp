...
BulletTestScene::BulletTestScene(sge::Spade* engine) : engine(engine), renderer(engine->getRenderer()), alpha(0.0f), useMouse(false), camSpeed(0.5f), played(false)
{
	...
	//-------------------------
	// !!!!! Warning takes a lot of time with bigger models
	fallShapeSuzanne = new btConvexHullShape();
	...
	spawnShape = new btBoxShape(btVector3(1, 1, 1));
	...
	btCylinderShape* treeShape = new btCylinderShape(btVector3(btScalar(0.6), btScalar(15.0), btScalar(0.4)));
...
BulletTestScene::~BulletTestScene()
{
	...
	dynamicsWorld->removeRigidBody(earthRigidBody);
	delete earthRigidBody->getMotionState();
	delete earthRigidBody;

	delete fallShape;
	delete simplifiedConvexShape;
	delete fallShapeSuzanne;
	delete spawnShape;
